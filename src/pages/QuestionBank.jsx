...
export default function QuestionBank() {
  ...
  const [activeTrack, setActiveTrack] = useState("LET");
  const [summary, setSummary] = useState({ LET: 0, CPA: 0 });
  ...
  useEffect(() => {
    ...
    if (trackFromUrl === "LET" || trackFromUrl === "CPA") {
  ...
  const loadQuestions = async () => {
    setFetching(true);
    try {
      const data = await listQuestionsApi(activeTrack);
      setQuestions(data || []);
      clearNotice();
    } catch (err) {
      showNotice({
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const payload = { ...form, exam_type: activeTrack };
      await createQuestionApi(payload);
      setForm((prev) => ({ ...prev, subject: "", topic: "", question: "", a: "", b: "", c: "", d: "" }));
      showNotice({
        type: "success",
  ...
  const handleClearAll = () => {
    showNotice({
      ...
      message: `This will remove every question in the ${activeTrack} bank. This action cannot be undone.`,
      actions: [
        {
        ...
        <div className="question-header">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div className="count-pill">
              LET: {summary.LET} | CPA: {summary.CPA}
            </div>
          </div>
        </div>
          ...
          <div className="question-panel">
            ...
            <div className="panel-box" style={{ marginTop: "14px" }}>
              ...
              {fetching ? (
                ...
                <p>No questions yet for {activeTrack}.</p>
              ) : (
          ...
          <div className="question-panel">
            <div className="panel-box panel-box--relative">
              ...
              <form onSubmit={handleSubmit}>
                <input type="hidden" value={activeTrack} onChange={(e) => handleChange("exam_type", e.target.value)} />
                <label>Subject</label>
                <input
                  value={form.subject}
